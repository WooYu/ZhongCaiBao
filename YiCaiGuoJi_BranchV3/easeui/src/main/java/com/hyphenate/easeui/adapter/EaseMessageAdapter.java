...
import com.hyphenate.easeui.widget.chatrow.EaseChatRowText2;
import com.hyphenate.easeui.widget.chatrow.EaseChatRowText3;
...
public class EaseMessageAdapter extends BaseAdapter{
	...
	private static final int MESSAGE_TYPE_SENT_TXTONE = 14;
	private static final int MESSAGE_TYPE_RECV_TXTONE = 15;
	private static final int MESSAGE_TYPE_SENT_TXTJONEROOM = 16;
	private static final int MESSAGE_TYPE_RECV_TXTJONEROOM = 17;
	...
	public int getItemViewType(int position) {
		...
		if (message.getType() == EMMessage.Type.TXT) {
			...
			if (message.getUserName().equals("8001")||message.getUserName().equals("8003")) {
				return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_TXTONE : MESSAGE_TYPE_SENT_TXTONE;
			}
			if (message.getUserName().equals("8002")) {
				return message.direct() == EMMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_TXTJONEROOM : MESSAGE_TYPE_SENT_TXTJONEROOM;
			}
	...
	protected EaseChatRow createChatRow(Context context, EMMessage message, int position) {
        ...
        case TXT:
            ...
            }else if (message.getUserName().equals("8001")||message.getUserName().equals("8003")) {//自定义8001布局的限制条件
				// 添加的佈局
				chatRow = new EaseChatRowText2(context, message, position, this);

			} else if (message.getUserName().equals("8002")) {//自定义8001布局的限制条件
				// 添加的佈局
				chatRow = new EaseChatRowText3(context, message, position, this);
